...
const ExtraRunSection = ({totalLBRuns, totalNBRuns, totalWDRuns, totalPRRuns, totalNRRuns, type}) => {
    return (
        <>
            {
                (totalLBRuns > 0 || totalNBRuns > 0 || totalWDRuns > 0 || totalPRRuns > 0 || totalNRRuns > 0) &&
                <Box className={`extra_run_section ${type === 'scorecard' && 'scores'}`}>
                    ...
                    <Typography variant="body2">{`Total: ${totalNBRuns + totalWDRuns + totalLBRuns + totalPRRuns - totalNRRuns}`}</Typography>
                </Box>
            }
        </>
    )
